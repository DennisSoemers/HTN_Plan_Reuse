...
UHTNTask_GetLoadedGunSimpleFPS::UHTNTask_GetLoadedGunSimpleFPS(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	TaskName = "Get Loaded Gun";

	// this task can sometimes be decomposed into an empty network, so heuristic of 0
	HeuristicCost = 0.f;
}

TArray<TSharedPtr<FHTNTaskInstance>> UHTNTask_GetLoadedGunSimpleFPS::FindDecompositions(UHTNPlannerComponent& HTNComp, 
																						...
																						uint8* TaskMemory)
{
	...
	if(const FHTNWorldState_SimpleFPS* SimpleFPSWorldState = static_cast<FHTNWorldState_SimpleFPS*>(WorldState.Get()))
	{
		...
		for(FSimpleFPSObject* Gun : WorldStateGuns)
		{
			...
			{
				if(SimpleFPSWorldState->IsReachable(NPCArea, GunArea))	// can reach gun from our current location
				{
					Guns.Add(Gun);
				}
			}
	...
	{
		UE_LOG(LogHTNPlanner, Error, TEXT("UHTNTask_GetLoadedGunSimpleFPS is only compatible with FHTNWorldState_SimpleFPS as World State class!"));
	}

	return Decompositions;
}

float UHTNTask_GetLoadedGunSimpleFPS::GetHeuristicCost() const
{
